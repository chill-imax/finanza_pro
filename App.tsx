...
const DEFAULT_ACCOUNTS: Account[] = [
  { id: '1', name: 'Efectivo', type: AccountType.CASH, currency: Currency.USD, balance: 150, color: '#10b981', icon: '💵' },
  { id: '2', name: 'Banesco', type: AccountType.BANK, currency: Currency.VES, balance: 5000, color: '#10b981', icon: '🏦' },
  { id: '3', name: 'Zelle', type: AccountType.WALLET, currency: Currency.USD, balance: 1200, color: '#8b5cf6', icon: '📱' },
  { id: '4', name: 'Ahorros', type: AccountType.SAVINGS, currency: Currency.USD, balance: 500, color: '#3b82f6', icon: '🐖' },
];
...
function App() {
  ...
  const totalBalanceUSD = accounts.filter(a => a.currency === Currency.USD).reduce((sum, a) => sum + a.balance, 0);
  const totalBalanceVES = accounts.filter(a => a.currency === Currency.VES).reduce((sum, a) => sum + a.balance, 0);
  ...
  const expensesData = useMemo(() => {
     ...
     transactions.filter(t => t.type === TransactionType.EXPENSE && t.currency === Currency.USD).forEach(t => {
  ...
  const renderDashboard = () => (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
        <button onClick={openNewTransaction} className="flex items-center gap-2 bg-primary text-white px-5 py-3 rounded-xl font-semibold shadow-lg whitespace-nowrap active:scale-95 transition-transform">
      ...
      {expensesData.length > 0 && (
         <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="font-bold text-lg text-slate-800 mb-4">Gastos (USD)</h3>
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-1 bg-red-50 p-4 rounded-xl border border-red-100">
             ...
             <p className="text-xl font-bold text-red-700 mt-1">
                ${debts.filter(d => d.type === 'I_OWE' && !d.isPaid && d.currency === 'USD').reduce((a, b) => a + (b.amount - b.paidAmount), 0)}
             </p>
        </div>
        <div className="flex-1 bg-emerald-50 p-4 rounded-xl border border-emerald-100">
             ...
             <p className="text-xl font-bold text-emerald-700 mt-1">
                ${debts.filter(d => d.type === 'OWES_ME' && !d.isPaid && d.currency === 'USD').reduce((a, b) => a + (b.amount - b.paidAmount), 0)}
             </p>
        </div>
      </div>
          ...
          <div key={debt.id} className={`bg-white p-4 rounded-xl shadow-sm border border-slate-100 ${debt.isPaid ? 'opacity-60' : ''}`}>
             <div className="flex justify-between items-start">
                 ...
                 <div className="flex flex-col items-end">
                    <p className="font-bold text-lg">{debt.currency === 'USD' ? '$' : 'Bs.'}{debt.amount}</p>
                    <button 
  ...
  const renderAdvisor = () => (
    <div className="space-y-6 pb-20">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-3xl text-white shadow-lg">
        ...
        <p className="text-blue-100 mt-2">Utiliza la IA para analizar tus finanzas en VES y USD y obtener recomendaciones personalizadas.</p>
