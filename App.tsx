...
import { Account, Transaction, Debt, Currency, TransactionType, AccountType, DEFAULT_CATEGORIES, Category, RecurringTransaction, Frequency } from './types';
...
import { Plus, LayoutDashboard, Wallet, ArrowRightLeft, TrendingUp, Bot, PiggyBank, Trash2, ArrowDownCircle, ArrowUpCircle, Flame, CheckCircle, RefreshCcw, Layers } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip as RechartsTooltip, Legend } from 'recharts';

function App() {
  // --- Country & Currency State ---
  const [userCountry, setUserCountry] = useState<string>(() => localStorage.getItem('user_country') || '');
  const [mainCurrency, setMainCurrency] = useState<string>(() => localStorage.getItem('main_currency') || 'USD');
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'accounts' | 'debts' | 'advisor'>('dashboard');
  ...
  const [confirmationData, setConfirmationData] = useState<{ isOpen: boolean; title: string; message: string; onConfirm: () => void; isDanger?: boolean }>({
      isOpen: false,
      title: '',
      message: '',
      onConfirm: () => {},
  ...
  useEffect(() => {
    const processRecurring = () => {
        ...
        const calculateNextDueDate = (current: string, rt: RecurringTransaction): string => {
            const date = new Date(current);
            switch (rt.frequency) {
                case Frequency.DAILY: date.setDate(date.getDate() + 1); break;
                case Frequency.WEEKLY: date.setDate(date.getDate() + 7); break;
                case Frequency.BIWEEKLY: date.setDate(date.getDate() + 15); break;
                ...
                case Frequency.CUSTOM:
                    ...
                    if (rt.customUnit === 'DAYS') date.setDate(date.getDate() + interval);
                    else if (rt.customUnit === 'WEEKS') date.setDate(date.getDate() + (interval * 7));
                    else if (rt.customUnit === 'MONTHS') date.setMonth(date.getMonth() + interval);
                    else if (rt.customUnit === 'YEARS') date.setFullYear(date.getFullYear() + interval);
                    break;
        ...
        if (hasChanges) {
            ...
            alert(`Se han generado ${newTransactions.length} transacciones recurrentes pendientes.`);
        }
    };
  ...
  const handleSetupComplete = (country: string, currency: string) => {
    ...
    if (accounts.length === 0) {
        ...
        if (country === 'Venezuela') {
            initialAccounts = [
                { id: '1', name: 'Efectivo USD', type: AccountType.CASH, currency: Currency.USD, balance: 0, color: '#10b981', icon: '💵' },
                { id: '2', name: 'Banesco', type: AccountType.BANK, currency: Currency.VES, balance: 0, color: '#3b82f6', icon: '🏦' },
                { id: '3', name: 'Zelle', type: AccountType.WALLET, currency: Currency.USD, balance: 0, color: '#8b5cf6', icon: '📱' },
            ];
        } else {
            // Para el resto de países, solo creamos cuentas con su moneda local
            initialAccounts = [
                { id: '1', name: 'Efectivo', type: AccountType.CASH, currency: currency as Currency, balance: 0, color: '#10b981', icon: '💵' },
                { id: '2', name: 'Banco', type: AccountType.BANK, currency: currency as Currency, balance: 0, color: '#3b82f6', icon: '🏦' },
                { id: '3', name: 'Ahorros', type: AccountType.SAVINGS, currency: currency as Currency, balance: 0, color: '#8b5cf6', icon: '🐖' },
            ];
        }
  ...
  const addTransaction = (data: any) => {
    ...
    if (data.isRecurring) {
        ...
        switch (freq) {
            case Frequency.DAILY: nextDate.setDate(nextDate.getDate() + 1); break;
            case Frequency.WEEKLY: nextDate.setDate(nextDate.getDate() + 7); break;
            case Frequency.BIWEEKLY: nextDate.setDate(nextDate.getDate() + 15); break;
            case Frequency.MONTHLY: nextDate.setMonth(nextDate.getMonth() + 1); break;
            case Frequency.YEARLY: nextDate.setFullYear(nextDate.getFullYear() + 1); break;
            case Frequency.CUSTOM:
    ...
    if (newTx.type === TransactionType.TRANSFER && newTx.toAccountId) {
        ...
        if (fromAcc && toAcc) {
            ...
            if (fromAcc.currency !== toAcc.currency && newTx.exchangeRate) {
                if (fromAcc.currency === Currency.USD && toAcc.currency === Currency.VES) {
                    transferAddAmount = newTx.amount * newTx.exchangeRate;
                }
                else if (fromAcc.currency === Currency.VES && toAcc.currency === Currency.USD) {
                    transferAddAmount = newTx.amount / newTx.exchangeRate;
                }
            }
        }
    ...
    const updatedAccounts = accounts.map(acc => {
      if (acc.id === newTx.accountId) {
        if (newTx.type === TransactionType.INCOME) return { ...acc, balance: acc.balance + newTx.amount };
        if (newTx.type === TransactionType.EXPENSE) return { ...acc, balance: acc.balance - newTx.amount };
        if (newTx.type === TransactionType.TRANSFER) return { ...acc, balance: acc.balance - newTx.amount };
      }
  ...
  const deleteTransaction = (id: string) => {
    ...
    const updatedAccounts = accounts.map(acc => {
         if (acc.id === tx.accountId) {
             if (tx.type === TransactionType.INCOME) return { ...acc, balance: acc.balance - tx.amount };
             if (tx.type === TransactionType.EXPENSE) return { ...acc, balance: acc.balance + tx.amount };
             if (tx.type === TransactionType.TRANSFER) return { ...acc, balance: acc.balance + tx.amount };
         }
         if (tx.type === TransactionType.TRANSFER && acc.id === tx.toAccountId) {
             ...
             if (tx.exchangeRate) {
                 ...
                 if (sourceAcc?.currency === Currency.USD && acc.currency === Currency.VES) addedAmount = tx.amount * tx.exchangeRate;
                 else if (sourceAcc?.currency === Currency.VES && acc.currency === Currency.USD) addedAmount = tx.amount / tx.exchangeRate;
             }
  ...
  const handleDeleteAccountAttempt = (account: Account) => {
      ...
      if (hasTransactions) {
          alert("No se puede eliminar esta cuenta porque tiene transacciones asociadas. Elimina las transacciones primero.");
          return;
      }
      setConfirmationData({
          isOpen: true,
  ...
  const handleDebtPayment = (amount: number, accountId: string, exchangeRate?: number) => {
    ...
    const updatedAccounts = accounts.map(acc => {
        if (acc.id === accountId) {
            ...
            if (acc.currency !== payDebtData.currency && exchangeRate) {
                if (payDebtData.currency === Currency.USD && acc.currency === Currency.VES) {
                    deductionAmount = amount * exchangeRate;
                }
                else if (payDebtData.currency === Currency.VES && acc.currency === Currency.USD) {
                    deductionAmount = amount / exchangeRate;
                }
            }
            ...
            return {
                ...acc,
                balance: isPaying ? acc.balance - deductionAmount : acc.balance + deductionAmount
            };
        }
        return acc;
    });
  ...
  const payYourselfFirst = () => {
    ...
    if(!savingsAcc) {
        alert("Para usar esta función, primero crea una cuenta del tipo 'Ahorros' en la sección Cuentas.");
        setActiveTab('accounts');
        return;
    }
    const sourceAcc = accounts.find(a => a.type === AccountType.BANK) || accounts.find(a => a.type === AccountType.WALLET) || accounts[0];
  ...
  const totalBalanceVES = accounts.filter(a => a.currency === 'VES').reduce((sum, a) => sum + a.balance, 0);
  ...
  const expensesData = useMemo(() => {
     ...
     transactions
       .filter(t => t.type === TransactionType.EXPENSE && (userCountry === 'Venezuela' ? t.currency === 'USD' : true))
       .forEach(t => {
  ...
  const renderDashboard = () => (
    <div className="space-y-6 pb-24">
      <div className="flex justify-between items-center bg-white p-3 rounded-xl shadow-sm border border-slate-100">
         ...
         {userCountry === 'Venezuela' ? (
           <>
             <div className="flex items-center gap-2">
                ...
                <div>
                   <p className="text-[10px] text-slate-500 font-bold uppercase">Tasa BCV</p>
                   <p className="font-bold text-slate-800">{bcvRate > 0 ? `Bs. ${bcvRate.toFixed(2)}` : 'Cargando...'}</p>
                </div>
             </div>
            ...
            <div className="flex items-center gap-2">
                ...
                <div>
                   <p className="text-[10px] text-slate-500 font-bold uppercase">País</p>
                   <p className="font-bold text-slate-800">{userCountry || 'Configurando...'}</p>
                </div>
            </div>
         )}
         ...
         {/* Racha (Global) */}
         <div className="flex items-center gap-2">
            <Flame className={`w-5 h-5 ${streakCount > 0 ? 'text-orange-500 fill-orange-500' : 'text-slate-300'}`} />
            <div>
               <p className="text-[10px] text-slate-500 font-bold uppercase">Racha</p>
      ...
      <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 p-6 rounded-3xl shadow-xl text-white relative overflow-hidden">
        ...
        <p className="text-slate-400 text-xs font-medium uppercase tracking-wider mb-2">Patrimonio Total Estimado</p>
        
        {/* Renderizado condicional del balance total */}
        {userCountry === 'Venezuela' ? (
           <>
            <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black">${grandTotalUSD.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                <span className="text-lg text-slate-400 font-medium">USD</span>
            </div>
            <div className="mt-4 flex gap-4 text-sm text-slate-300">
                <div>
                    <span className="block text-[10px] uppercase text-slate-500">En Dólares</span>
                    <span className="font-semibold">${totalBalanceUSD.toLocaleString()}</span>
                </div>
                <div className="w-px bg-white/10"></div>
                <div>
                    <span className="block text-[10px] uppercase text-slate-500">En Bolívares</span>
                    <span className="font-semibold">Bs. {totalBalanceVES.toLocaleString()}</span>
                </div>
            </div>
           </>
        ) : (
            <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black">{totalBalanceGlobal.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
                <span className="text-lg text-slate-400 font-medium ml-1">{mainCurrency}</span>
            </div>
        )}
      </div>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
        {lastLogDate !== today && (
           <button onClick={skipLogDay} className="flex items-center gap-2 bg-slate-100 text-slate-600 px-5 py-3 rounded-xl font-semibold whitespace-nowrap active:scale-95 transition-transform border border-slate-200">
             <CheckCircle className="w-5 h-5" /> Sin Movimientos
           </button>
        )}
        <button onClick={payYourselfFirst} className="flex items-center gap-2 bg-emerald-100 text-emerald-700 px-5 py-3 rounded-xl font-semibold whitespace-nowrap active:scale-95 transition-transform">
      ...
      <div>
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setIsTxListModalOpen(true)} className="text-blue-600 text-sm font-medium hover:underline">Ver todo</button>
        </div>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
          {transactions.length === 0 ? (
            <div className="p-8 text-center text-slate-400">Sin transacciones aún</div>
          ) : (
            transactions.slice(0, 5).map(tx => {
               const cat = categories.find(c => c.id === tx.categoryId);
               return (
                  <div key={tx.id} onClick={() => setSelectedTransaction(tx)} className="flex items-center justify-between p-4 border-b last:border-0 hover:bg-slate-50 cursor-pointer">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-bold text-slate-800 text-sm">{cat ? cat.name : (tx.type === 'TRANSFER' ? 'Transferencia' : 'Otro')}</p>
                    ...
                    <span className={`font-bold ${tx.type === 'INCOME' ? 'text-emerald-600' : 'text-slate-800'}`}>
                       {tx.type === 'INCOME' ? '+' : '-'}{userCountry === 'Venezuela' ? (tx.currency === 'USD' ? '$' : 'Bs.') : mainCurrency + ' '}{tx.amount.toLocaleString()}
                    </span>
                  </div>
               );
            })
      ...
      {expensesData.length > 0 && (
         <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
            <h3 className="font-bold text-lg text-slate-800 mb-4">Gastos ({userCountry === 'Venezuela' ? 'USD' : mainCurrency})</h3>
        ...
        <button 
            onClick={() => {
                setEditingAccount({
                    ...
                    currency: (userCountry === 'Venezuela' ? 'USD' : mainCurrency) as Currency,
                    balance: 0,
                    color: '#3b82f6',
                    icon: '🏦'
                });
  ...
  const renderDebts = () => (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex gap-2 mb-4">
        <div className="flex-1 bg-red-50 p-4 rounded-xl border border-red-100">
             ...
             <p className="text-xl font-bold text-red-700 mt-1">
                ...
                {debts.filter(d => d.type === 'I_OWE' && !d.isPaid && (userCountry === 'Venezuela' ? d.currency === 'USD' : true)).reduce((a, b) => a + (b.amount - b.paidAmount), 0)}
             </p>
        </div>
        <div className="flex-1 bg-emerald-50 p-4 rounded-xl border border-emerald-100">
             ...
             <p className="text-xl font-bold text-emerald-700 mt-1">
                {userCountry !== 'Venezuela' ? mainCurrency + ' ' : '$'}
                {debts.filter(d => d.type === 'OWES_ME' && !d.isPaid && (userCountry === 'Venezuela' ? d.currency === 'USD' : true)).reduce((a, b) => a + (b.amount - b.paidAmount), 0)}
             </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {debts.map(debt => (
          <div key={debt.id} className={`bg-white p-4 rounded-xl shadow-sm border border-slate-100 ${debt.isPaid ? 'opacity-60' : ''}`}>
             <div className="flex justify-between items-start">
                 <div>
                    <h4 className="font-bold text-slate-800">{debt.name}</h4>
                    <span className={`text-xs px-2 py-0.5 rounded font-medium ${debt.type === 'I_OWE' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-600'}`}>
                        {debt.type === 'I_OWE' ? 'Debo' : 'Me deben'}
                    </span>
                 </div>
                 <div className="flex flex-col items-end">
                    <p className="font-bold text-lg">
                       {userCountry === 'Venezuela' ? (debt.currency === 'USD' ? '$' : 'Bs.') : mainCurrency + ' '}{debt.amount}
                    </p>
                    <button 
        ...
        {debts.length === 0 && <p className="text-center text-slate-400 py-8">¡Estás libre de deudas!</p>}
  ...
  const renderAdvisor = () => (
    <div className="space-y-6 pb-24">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-3xl text-white shadow-lg">
        ...
        <p className="text-blue-100 mt-2">Utiliza la IA para analizar tus finanzas y obtener recomendaciones personalizadas.</p>
      ...
      <TransactionDetailModal 
        ...
        onDelete={(id) => {
            setConfirmationData({
                ...
                message: '¿Estás seguro de que deseas eliminar esta transacción? Los saldos serán revertidos.',
                isDanger: true,
