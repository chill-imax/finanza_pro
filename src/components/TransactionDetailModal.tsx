    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden shadow-2xl animate-fade-in">
        ...
        <div className="p-6 text-center border-b border-slate-50">
          <div
            className={`mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-3 text-3xl ${transaction.type === "INCOME" ? "bg-emerald-100" : transaction.type === "TRANSFER" ? "bg-blue-100" : "bg-red-100"}`}
          >
          ...
          <p
            className={`text-3xl font-black mt-1 ${transaction.type === "INCOME" ? "text-emerald-600" : "text-slate-800"}`}
          >
            {transaction.currency === "USD" ? "$" : "Bs."}{" "}
          ...
          {transaction.currency === "VES" &&
            ...
            transaction.exchangeRate > 0 && (
              <p className="text-sm font-bold text-slate-400 mt-1">
                ≈ $
                {(transaction.amount / transaction.exchangeRate).toLocaleString(
                  undefined,
                  { maximumFractionDigits: 2 },
                )}{" "}
                USD
          ...
          {isDebtPayment && (
            <div className="flex items-start gap-2 p-3 bg-amber-50 rounded-lg border border-amber-100">
              ...
              <p className="text-xs text-amber-700">
                Por motivos de consistencia en los saldos, los abonos a deudas
                no pueden ser editados. Solo puedes eliminarlos.
              </p>
            </div>
          )}
        </div>
