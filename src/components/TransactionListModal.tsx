...
export const TransactionListModal: React.FC<Props> = ({
  isOpen,
  onClose,
  transactions,
  categories,
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg h-[80vh] flex flex-col shadow-2xl animate-fade-in">
        ...
        <div className="p-4 flex gap-2 overflow-x-auto shrink-0 border-b border-slate-50">
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${filterType === "ALL" ? "bg-slate-800 text-white" : "bg-slate-100 text-slate-600"}`}
          >
            Todos
          </button>
          <button
            onClick={() => setFilterType(TransactionType.INCOME)}
            className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${filterType === TransactionType.INCOME ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-600"}`}
          >
            Ingresos
          </button>
          <button
            onClick={() => setFilterType(TransactionType.EXPENSE)}
            className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${filterType === TransactionType.EXPENSE ? "bg-red-500 text-white" : "bg-slate-100 text-slate-600"}`}
          >
            Gastos
          </button>
          <button
            onClick={() => setFilterType(TransactionType.TRANSFER)}
            className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${filterType === TransactionType.TRANSFER ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-600"}`}
          >
            Transferencias
          </button>
        </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-500">
                      {tx.date} • {getAccountName(tx.accountId)}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <span
                    className={`font-bold block ${tx.type === "INCOME" ? "text-emerald-600" : "text-slate-800"}`}
                  >
                    {tx.type === "INCOME" ? "+" : "-"}
                    {tx.currency === "USD" ? "$" : "Bs."}
