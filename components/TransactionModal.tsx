      ...
      <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs space-y-1 mt-4">
        <div className="flex justify-between">
          ...
          <span className="font-bold text-red-600">
            - {sourceAccount.currency === "USD" ? "$" : "Bs."}
            {sourceDeduction.toLocaleString(undefined, {
        ...
        {type === TransactionType.TRANSFER && targetAccount && (
          <div className="flex justify-between pt-1 border-t border-slate-200">
            ...
            <span className="font-bold text-emerald-600">
              + {targetAccount.currency === "USD" ? "$" : "Bs."}
              {targetAddition.toLocaleString(undefined, {
                maximumFractionDigits: 2,
              })}
            </span>
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto no-scrollbar">
        {/* CABECERA DINÁMICA — corregido: todos los hijos dentro del mismo div */}
        <div
          className={`flex justify-between items-center p-4 border-b transition-colors ${isSavings ? "bg-green-600 border-emerald-600" : "bg-white border-slate-200"}`}
        >
          <h2
            className={`text-lg font-bold ${isSavings ? "text-white" : "text-slate-800"}`}
          >
            {isSavings ? "Págate a ti mismo 🌱" : "Nueva Transacción"}
          </h2>
          <button
            onClick={onClose}
            className={`p-2 rounded-full transition-colors ${isSavings ? "hover:bg-emerald-600" : "hover:bg-slate-100"}`}
          >
            <X
              className={`w-5 h-5 ${isSavings ? "text-white" : "text-slate-500"}`}
            />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          {/* OCULTAR BOTONES SI ESTÁ EN MODO AHORRO */}
          {!isSavings && (
          ...
          {type !== TransactionType.TRANSFER && (
            <div>
              {/* corregido: label no puede tener <button> como hijo directo junto a <span>, se usa div wrapper */}
          ...
          <button
            type="submit"
            className={`w-full py-3 font-bold rounded-xl shadow-lg transition-colors mt-4 ${
              isSavings
